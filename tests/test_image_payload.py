...
def test_payload_user_content_keeps_text_delivery() -> None:
    scenario = load_named_template("AML.T0085.001_cross-user-portfolio", Path("attacks"))
...
def test_payload_user_content_wraps_image_delivery(tmp_path: Path, monkeypatch) -> None:
    scenario = load_named_template(
        "AML.T0080.000_memory-poisoning-image-injection", default_attacks_dir()
    )
