...
def test_catalog_contains_generic_templates_sorted_by_name() -> None:
    ...
    assert "AML.T0051.000_openclaw-goal-hijack" in names
...
def test_named_attack_requires_and_applies_profile() -> None:
    ...
    scenario = load_named_attack("AML.T0080.000_memory-poisoning", Path("attacks"), profile)
...
def test_explicit_template_path_loads_without_target_specific_config() -> None:
    template = load_named_template(
        "attacks/AML.T0085.001_cross-user-portfolio.yaml", Path("missing")
    )
    assert template.name == "AML.T0085.001_cross-user-portfolio"
...
def test_missing_slot_and_endpoint_are_skipped() -> None:
    template = load_named_template("AML.T0085.001_cross-user-portfolio", Path("attacks"))
    profile = _profile(
        "AML.T0085.001_cross-user-portfolio",
    ...
    profile.bindings["AML.T0085.001_cross-user-portfolio"]["target"]["endpoint"] = None
    instances, skipped = apply_profile([template], profile)
    assert instances == []
