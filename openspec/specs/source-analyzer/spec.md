...
Команда `inspect` читает исходники стенда через анализатор и пишет StandProfile в YAML без HTTP к живому стенду.
