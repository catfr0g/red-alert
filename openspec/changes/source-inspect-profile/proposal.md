...
- `red-alert attack` читает профиль, собирает экземпляры (как нынешний `AttackScenario`) и гоняет существующий граф. **BREAKING**: `--profile` / `RED_ALERT_PROFILE` обязателен; упакованный `invest-stand` — для демо/тестов, не default CLI.
